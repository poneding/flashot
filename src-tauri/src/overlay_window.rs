...
#[derive(Clone, Copy)]
pub struct ScrollIndicatorColor {
    pub red: u8,
    pub green: u8,
    pub blue: u8,
    pub alpha: u8,
...
pub fn show_wayland_scroll_indicator(
    app: &AppHandle,
    monitor_id: u32,
    monitor_rect: Rect,
    selection: Rect,
    frozen_frame: ScrollOverlayFrame,
    color: ScrollIndicatorColor,
) -> Result<()> {
    show_platform_wayland_scroll_indicator(
        app,
        monitor_id,
        monitor_rect,
        selection,
...
pub fn hide_wayland_scroll_indicator(app: &AppHandle, monitor_id: u32) -> Result<()> {
    hide_platform_wayland_scroll_indicator(app, monitor_id)
}

pub fn hide_all_wayland_scroll_indicators(app: &AppHandle) -> Result<()> {
    hide_all_platform_wayland_scroll_indicators(app)
}
...
#[cfg(target_os = "linux")]
fn show_platform_wayland_scroll_indicator(
    app: &AppHandle,
    monitor_id: u32,
    monitor_rect: Rect,
    selection: Rect,
    ...
    run_on_app_main_thread(app, "show wayland scroll indicator", move || {
        show_wayland_scroll_indicator_on_main(
            monitor_id,
            monitor_rect,
            selection,
            frozen_frame,
...
fn show_platform_wayland_scroll_indicator(
    ...
    _monitor_rect: Rect,
    _selection: Rect,
    _frozen_frame: ScrollOverlayFrame,
    _color: ScrollIndicatorColor,
) -> Result<()> {
    Ok(())
}

#[cfg(target_os = "linux")]
fn hide_platform_wayland_scroll_indicator(app: &AppHandle, monitor_id: u32) -> Result<()> {
    if !is_linux_wayland_session() {
        return Ok(());
    }

    run_on_app_main_thread(app, "hide wayland scroll indicator", move || {
        hide_wayland_scroll_indicator_on_main(monitor_id);
        Ok(())
    })
}

#[cfg(not(target_os = "linux"))]
fn hide_platform_wayland_scroll_indicator(_app: &AppHandle, _monitor_id: u32) -> Result<()> {
    Ok(())
}

#[cfg(target_os = "linux")]
fn hide_all_platform_wayland_scroll_indicators(app: &AppHandle) -> Result<()> {
    if !is_linux_wayland_session() {
        return Ok(());
    }

    run_on_app_main_thread(app, "hide all wayland scroll indicators", move || {
        hide_all_wayland_scroll_indicators_on_main();
        Ok(())
    })
}

#[cfg(not(target_os = "linux"))]
fn hide_all_platform_wayland_scroll_indicators(_app: &AppHandle) -> Result<()> {
    Ok(())
}
...
#[cfg(target_os = "linux")]
thread_local! {
    static WAYLAND_SCROLL_INDICATORS: std::cell::RefCell<std::collections::HashMap<u32, gtk::Window>> =
        std::cell::RefCell::new(std::collections::HashMap::new());
}

#[cfg(target_os = "linux")]
fn show_wayland_scroll_indicator_on_main(
    monitor_id: u32,
    monitor_rect: Rect,
    selection: Rect,
    frozen_frame: ScrollOverlayFrame,
    ...
    hide_wayland_scroll_indicator_on_main(monitor_id);
    let frozen_surface = create_scroll_overlay_surface(frozen_frame)?;

    let window = gtk::Window::new(gtk::WindowType::Popup);
    window.set_title("Flashot Scroll Region");
    ...
    let frozen_tiles = outside_selection_tiles(monitor_rect, selection);
    let border_segments = scroll_indicator_segments(monitor_rect, selection, 2);
    let surface_width = frozen_surface.width().max(1) as f64;
    let surface_height = frozen_surface.height().max(1) as f64;
    let scale_x = monitor_rect.width.max(1) as f64 / surface_width;
    let scale_y = monitor_rect.height.max(1) as f64 / surface_height;
    area.connect_draw(move |_, cr| {
        ...
        for tile in &frozen_tiles {
            ...
            let _ = cr.set_source_surface(&frozen_surface, 0.0, 0.0);
            let _ = cr.paint();
            let _ = cr.restore();
        }
    ...
    window.add(&area);
    configure_wayland_scroll_indicator_window(&window, monitor_id, monitor_rect)?;
    window.connect_realize(move |window| {
        if let Some(gdk_window) = window.window() {
            let input_region = scroll_overlay_input_region(monitor_rect, selection);
    ...
    WAYLAND_SCROLL_INDICATORS.with(|indicators| {
        indicators.borrow_mut().insert(monitor_id, window);
    });
...
#[cfg(target_os = "linux")]
fn create_scroll_overlay_surface(frame: ScrollOverlayFrame) -> Result<gtk::cairo::ImageSurface> {
    if frame.width == 0 || frame.height == 0 {
        anyhow::bail!("wayland scroll overlay frame must not be empty");
    }
    let expected_len = frame.width as usize * frame.height as usize * 4;
    if frame.rgba.len() != expected_len {
        anyhow::bail!("wayland scroll overlay frame has an invalid buffer size");
    }

    let data = rgba_to_cairo_argb32(&frame.rgba);
    ...
    .map_err(|e| anyhow!("failed to create wayland scroll overlay surface: {e:?}"))
}
...
fn rgba_to_cairo_argb32(rgba: &[u8]) -> Vec<u8> {
    ...
    for px in rgba.chunks_exact(4) {
        ...
        let premultiply =
            |channel: u8| -> u8 { ((channel as u16 * alpha + 127) / 255).min(255) as u8 };
        let red = premultiply(px[0]);
        let green = premultiply(px[1]);
        let blue = premultiply(px[2]);

        // Cairo ARgb32 uses native-endian premultiplied ARGB. On Linux's
        // little-endian targets that is stored in memory as BGRA.
        out.extend_from_slice(&[blue, green, red, px[3]]);
    }
    out
}

#[cfg(target_os = "linux")]
fn configure_wayland_scroll_indicator_window(
    window: &gtk::Window,
    monitor_id: u32,
    monitor_rect: Rect,
) -> Result<()> {
    ...
    let Some(layer_shell) = linux_layer_shell() else {
        configure_wayland_scroll_indicator_fullscreen_fallback(window, monitor_rect);
        return Ok(());
    };
    ...
    let namespace = CString::new(format!("flashot-scroll-indicator-{monitor_id}"))?;
    layer_shell.set_namespace(gtk_ptr, namespace.as_ptr());
    layer_shell.set_layer(gtk_ptr, GTK_LAYER_SHELL_LAYER_OVERLAY);
    layer_shell.set_exclusive_zone(gtk_ptr, 0);
    ...
    if let Some((monitor, _index)) = gdk_monitor_for_display_rect(&display, monitor_rect) {
        ...
        tracing::warn!("failed to map scroll indicator monitor {monitor_id} to a GDK monitor");
    }
...
#[cfg(target_os = "linux")]
fn configure_wayland_scroll_indicator_fullscreen_fallback(window: &gtk::Window, monitor_rect: Rect) {
    use gtk::prelude::*;

    tracing::warn!(
        "Wayland compositor does not support layer-shell; using native scroll indicator fullscreen fallback"
    );

    let display = window.display();
    if let (Some(screen), Some((_monitor, index))) = (
        gtk::prelude::GtkWindowExt::screen(window),
        gdk_monitor_for_display_rect(&display, monitor_rect),
    ) {
...
#[cfg(target_os = "linux")]
fn hide_wayland_scroll_indicator_on_main(monitor_id: u32) {
    use gtk::prelude::*;

    WAYLAND_SCROLL_INDICATORS.with(|indicators| {
        if let Some(window) = indicators.borrow_mut().remove(&monitor_id) {
            window.close();
        }
    });
}
...
#[cfg(target_os = "linux")]
fn hide_all_wayland_scroll_indicators_on_main() {
    use gtk::prelude::*;

    WAYLAND_SCROLL_INDICATORS.with(|indicators| {
        for (_monitor_id, window) in indicators.borrow_mut().drain() {
            window.close();
        }
    });
}
...
#[cfg(target_os = "linux")]
fn scroll_overlay_input_region(monitor_rect: Rect, selection: Rect) -> gtk::cairo::Region {
    let rects = outside_selection_tiles(monitor_rect, selection)
        .into_iter()
        .map(|rect| {
            gtk::cairo::RectangleInt::new(rect.x, rect.y, rect.width as i32, rect.height as i32)
        })
        .collect::<Vec<_>>();
...
#[cfg(any(target_os = "linux", test))]
fn outside_selection_tiles(monitor_rect: Rect, selection: Rect) -> Vec<Rect> {
    ...
    push_positive_rect(
        ...
        Rect {
            ...
            y: bottom,
            width: monitor_rect.width,
            height: (mon_height - bottom) as u32,
        },
    );
    push_positive_rect(
        &mut tiles,
...
#[cfg(any(target_os = "linux", test))]
fn scroll_indicator_segments(monitor_rect: Rect, selection: Rect, thickness: u32) -> Vec<Rect> {
    let mon_width = monitor_rect.width as i32;
    let mon_height = monitor_rect.height as i32;
    let left = selection.x.clamp(0, mon_width);
    let top = selection.y.clamp(0, mon_height);
    ...
    if bottom_y < bottom_edge {
        segments.push(Rect {
            x: left,
            y: bottom_y,
            width: (right - left) as u32,
            height: (bottom_edge - bottom_y) as u32,
        });
    }
    ...
    if right_x < right_edge {
        segments.push(Rect {
            x: right_x,
            y: top,
            width: (right_edge - right_x) as u32,
            height: (bottom - top) as u32,
        });
    }
...
mod tests {
    ...
    #[test]
    fn wayland_scroll_indicator_segments_stay_outside_selection() {
        let segments = super::scroll_indicator_segments(
            Rect {
                x: 10,
                y: 20,
                width: 800,
    ...
    #[test]
    fn wayland_scroll_overlay_freezes_outside_selection_only() {
        let tiles = super::outside_selection_tiles(
            Rect {
                x: 10,
                y: 20,
                width: 800,
    ...
    #[test]
    fn wayland_scroll_overlay_converts_rgba_to_cairo_argb32() {
        let converted = super::rgba_to_cairo_argb32(&[10, 20, 30, 255, 200, 100, 50, 128]);

        assert_eq!(converted, vec![30, 20, 10, 255, 25, 50, 100, 128]);
    }

    #[test]
    fn wayland_scroll_indicator_is_native_selection_hole_layer() {
        ...
        assert!(production.contains("show_wayland_scroll_indicator"));
        assert!(production.contains("gtk::DrawingArea"));
        assert!(production.contains("outside_selection_tiles"));
        assert!(production.contains("input_shape_combine_region"));
        assert!(production.contains("set_pass_through(false)"));
        assert!(production.contains("GTK_LAYER_SHELL_KEYBOARD_MODE_NONE"));
        assert!(production.contains("fullscreen_on_monitor"));
        assert!(!production.contains("scroll-outline"));
        assert!(!production.contains("overlay-outline"));
    }
